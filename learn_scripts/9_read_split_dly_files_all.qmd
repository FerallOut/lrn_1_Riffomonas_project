...
# extract info from one of the split files

```{r}
#| label: info_extraction_for_one_file

## to remove more unnecessary data, decide on the time window you need for your calculations
window <- 100 # days

## now we can read directly from the tar file, which is also smaller
#data_in <-
read_fwf("../data/temp/xac.gz",
  ...
  mutate(julian_day = yday(date),
         ...
         year = year(date),
         year = if_else(diff_date < 0 & is_in_window, year + 1, year) ) %T>%
  ...
  filter(is_in_window) %>%
  group_by(id, year) %>%
  summarize(prcp = sum(prcp), .groups = "drop") %T>%
  {head(.) %>% print()} %>%
  {dim(.) %>% print()}
```
